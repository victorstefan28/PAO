...
class BooleanCalculatorResult extends CalculatorResult {
    ...
    @Override
    public Object computeResult() {
        boolean leftOperand = Boolean.parseBoolean(calculatorRequest.getLeftOperand().toString());
        boolean rightOperand = Boolean.parseBoolean(calculatorRequest.getRightOperand().toString());
        switch (calculatorRequest.getOperation()) {
            case "AND":
                return leftOperand && rightOperand;
            case "OR":
